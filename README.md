...
## OpenRGB
After a bit of work, the latest version of OpenRGB can be forced into running. In order to force it to run, launch it with 'DYLD_FORCE_FLAT_NAMESPACE=1 DYLD_INSERT_LIBRARIES=/opt/local/lib/libcxx/libc++.1.dylib', then replace as many of its prebundled libraries with libraries that you can install from MacPorts. As long as the version matches, as an example let's say you have 'libmbedx509.2.28.10.dylib'. In OpenRGB's frameworks folder, the same library might be under the name of 'libmbedx509.7.dylib'. After replacing every library and forcing modern 'libc++.1.dylib', it should run, and from there run the app patcher to get rid of the prohibited symbol over the icon. If you want it to be a perfectly normal looking application, make an automator script exported as an application and use my stub Info.plist file.
